 ...
 *
 * Running as an add-on, the Supervisor writes the user's settings to
 * /data/options.json rather than passing environment variables. Reading it
 * here means the add-on needs no shell, no bashio and no wrapper script - it
 * is the same binary, in the same scratch-thin image, reading one more file.
 *
 * Keys are the environment variable names without the prefix, lowercased:
 * THERMIQ_MQTT_HOST is "mqtt_host". An environment variable always wins, so
 * anything can still be overridden at run time.
 */

#define OPTIONS_MAX 32
#define OPTIONS_ARENA 2048

static struct {
    const char *keys[OPTIONS_MAX];
    const char *values[OPTIONS_MAX];
    int count;
    char arena[OPTIONS_ARENA];
    size_t used;
    char source[4096];
} options;

static const char *arena_printf(const char *fmt, double number)
{
    char rendered[64];
    int length = snprintf(rendered, sizeof(rendered), fmt, number);
    if (length <= 0 || options.used + (size_t)length + 1 > OPTIONS_ARENA)
        return NULL;
    char *out = options.arena + options.used;
    memcpy(out, rendered, (size_t)length + 1);
    options.used += (size_t)length + 1;
    return out;
}

static void options_load(const char *path)
{
    /* Reset first. The keys point into options.source, so leaving entries
     * from a previous call behind would leave them pointing into a buffer
     * this one is about to overwrite. Only the tests load twice, but a table
     * of dangling pointers is not something to leave lying about. */
    options.count = 0;
    options.used = 0;
    options.source[0] = '\0';
    ...
    fclose(file);
    if (!complete) {
        log_warn("%s is larger than expected, ignoring it", path);
        return;
    }
    options.source[length] = '\0';

    static struct json_member members[JSON_MAX_MEMBERS];
    int count = json_parse_object(options.source, length, members, JSON_MAX_MEMBERS);
    if (count < 0) {
        log_warn("%s is not a JSON object, ignoring it", path);
        return;
    }

    for (int i = 0; i < count && options.count < OPTIONS_MAX; i++) {
        const char *value = NULL;
        switch (members[i].kind) {
        case JSON_STRING:
            value = members[i].string;
            break;
        case JSON_BOOL:
            value = members[i].boolean ? "true" : "false";
            break;
        case JSON_NUMBER:
            value = members[i].is_integer ? arena_printf("%.0f", members[i].number)
                                          : arena_printf("%g", members[i].number);
            break;
        default:
            continue; /* null, or a nested value: nothing to configure with */
        }
        if (!value || !*value)
            continue; /* an empty option means "unset", not "empty string" */
        options.keys[options.count] = members[i].key;
        options.values[options.count] = value;
        options.count++;
    }
    log_info("read %d option(s) from %s", options.count, path);
}

/* THERMIQ_MQTT_HOST -> mqtt_host */
static const char *option_for(const char *name)
{
    static const char prefix[] = "THERMIQ_";
    ...
    for (int i = 0; i < options.count; i++) {
        ...
        size_t j = 0;
        for (; key[j] && suffix[j]; j++) {
            char lowered = suffix[j];
...
bool config_load(struct config *config, char *error, size_t error_cap)
{
    ...
    /* Home Assistant add-on settings, when running as one. Overridable with
     * THERMIQ_OPTIONS_FILE, which is also how the tests reach it. */
    const char *options_file = getenv("THERMIQ_OPTIONS_FILE");
    options_load(options_file && *options_file ? options_file : "/data/options.json");
