...
`vendor/heatpump_widget.j2` is the animated
...
Usage:
    python3 codegen/gen_widget.py [--check]
"""
...
TEMPLATE = REPO / "vendor/heatpump_widget.j2"
OUTPUT = REPO / "src/widget_gen.c"
OUTPUT_H = REPO / "src/widget_gen.h"
...
def generate() -> tuple[str, str]:
    ...
    generated = (
        ...
        " *\n"
        " * Source of truth:\n"
        " * vendor/heatpump_widget.j2\n"
        " *\n"
        " * Regenerate with:\n"
        " *     python3 codegen/gen_widget.py\n"
        " */\n"
        "#include <math.h>\n"
        "#include <string.h>\n"
        "\n"
    ...
    # WIDGET_ENTITY_COUNT is an extern const int, so nothing can size an array
    # with it. Callers were sizing theirs by hand, against a number only this
    # script knows - emit it as a macro so a template that grows past their
    # arrays fails to compile instead of running off the end of one.
    header = (
        "/* @generated by codegen/gen_widget.py - do not edit. */\n"
        ...
        f"#define WIDGET_ENTITY_MAX {len(compiler.entities)}\n"
        "\n"
    ...
    if args.check:
        ...
        if current != generated or current_h != header:
            print(
                ...
                f"Run: python3 codegen/gen_widget.py",
                file=sys.stderr,
            )
            return 1
